...
public final class QueryUtils {
    ...
    public static ArrayList<Quake> extractEarthquakes() {
        ...
        /** Convert the SAMPLE_JSON_RESPONSE string into a JSONObject */
            JSONObject quakeObject = new JSONObject(SAMPLE_JSON_RESPONSE);

            /** Extract the "features JSONArray from quakeObject */
            JSONArray quakesArray = quakeObject.getJSONArray("features");

            /** Loop through the array extracting all the info*/
            for (int i = 0; i < quakesArray.length(); i++){
                /** Get the earthquake JSONObject at position i */
                JSONObject currentQuake = quakesArray.getJSONObject(i);
                /** Get "properties" JSONObject */
                JSONObject quakeProperties = currentQuake.getJSONObject("properties");
                /** Extract "mag" for magnitude */
                double quakeMagnitude = quakeProperties.getDouble("mag");
                /** Extract "place" for location */
                String quakeLocation = quakeProperties.getString("place");
                /** Extract "time" for time */
                long quakeTime = quakeProperties.getLong("time");
                /** Extract "url" for url */
                String quakeUrl = quakeProperties.getString("url");
                /** Create earthquake java object from quakeMagnitude, quakeLocation and quakeTime */
                Quake earthquake = new Quake(quakeMagnitude, quakeLocation, quakeTime, quakeUrl);
                /** Add the earthquake object to the earthquakes Arraylist */
